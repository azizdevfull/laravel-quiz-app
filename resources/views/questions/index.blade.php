            ...
            <tbody>
                @foreach($questions as $question)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('quizzes.questions.show', [$quiz->id, $question->id]) }}" class="btn btn-info">View</a>
                            <a href="{{ route('quizzes.questions.edit', [$quiz->id, $question->id]) }}" class="btn btn-primary">Edit</a>
                            <form action="{{ route('quizzes.questions.destroy', [$quiz->id, $question->id]) }}" method="POST" style="display: inline-block;">
                                @csrf
                                @method('DELETE')
